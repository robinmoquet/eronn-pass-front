 ...
 */
export const path = (routeName: string, options: {[key: string]: string} | null = null, paramMustBeReplace: boolean = true): string => {
    ...
    if (routeHaveParams(route)) {
        ...
        route = replaceParams(route, params, options as {[key: string]: string}, paramMustBeReplace);
        
        if (paramMustBeReplace && hasRestOptions(options as {[key: string]: string}, params)) {
            route = addRestOptionsToQueryString(route, options as {[key: string]: string}, params);
        }
    }
...
const hasRestOptions = (options: {[key: string]: string}, params: Array<string>): boolean => {
    return Object.keys(options).filter((option: string) => !params.includes(option)).length > 0;
}
...
const replaceParams = (route: string, params: Array<string>, options: {[key: string]: string}, paramMustBeReplace: boolean): string => {
...
const addRestOptionsToQueryString = (route: string, options: {[key: string]: string}, params: Array<string>): string => {
    const restOptions = Object.keys(options).filter((option: string) => !params.includes(option));
    let res: string = route;
